...
(function () {
  ...
  var S = window.RADAR_STORIES,
    SRC = window.RADAR_SOURCES,
    TOPICS = window.RADAR_TOPICS;

  var GRADE = {
    primary: {
      bars: 4,
      word: "Primary source",
      def: "The company, lab or author published it themselves.",
    },
    corroborated: {
      bars: 3,
      word: "Corroborated",
      def: "Two or more independent outlets report the same thing.",
    },
    emerging: {
      bars: 2,
      word: "Emerging",
      def: "One outlet so far. Probably true, not yet confirmed.",
    },
    unverified: {
      bars: 1,
      word: "Unverified",
      def: "A rumour, a leak or an anonymous claim. Read it as such.",
    },
  };
  ...
  function verChip(level) {
    return (
      '<span class="ver">' + meter(level) + "<span>" + esc(GRADE[level].word) + "</span></span>"
    );
  }
  ...
  function spark(pickup, bare) {
    ...
    var svg =
      '<svg viewBox="0 0 ' +
      W +
      ' 40" role="img" aria-label="Outlets reporting this story rose from ' +
      pickup[0] +
      " to " +
      mx +
      " over " +
      hours +
      ' hours."><title>Cumulative outlets reporting this story</title>' +
      '<path d="' +
      d +
      " L" +
      last.x.toFixed(1) +
      ...
      " L6," +
      base +
      ' Z" fill="currentColor" opacity=".09"/>' +
      '<path d="' +
      d +
    ...
    return (
      '<div class="spark"><div class="spark-hd"><b>Pickup</b><span>' +
      mx +
      " outlet" +
      (mx === 1 ? "" : "s") +
      " \u00b7 " +
  ...
  function totalArrivals() {
    return window.RADAR_ARRIVALS.reduce(function (p, c) {
      return p + c;
    }, 0);
  }

  function arrivals(bare) {
    var a = window.RADAR_ARRIVALS,
      now = window.RADAR_CURRENT_HOUR;
    ...
    var total = a.reduce(function (p, c) {
      return p + c;
    }, 0);
    var ticks = [
      ["00", 2],
      ...
      .map(function (t) {
        return (
          '<text x="' +
          t[1] +
          '" y="57" font-family="JetBrains Mono, monospace" font-size="8" fill="#6E6A61">' +
          t[0] +
          "</text>"
        );
      })
      .join("");
    var asvg =
      '<svg viewBox="0 0 292 60" role="img" aria-label="Items ingested per hour today, ' +
      total +
      " in total, peaking at " +
      mx +
      " in the " +
      now +
      ':00 hour."><title>Items per hour, 00:00 to 23:00</title>' +
      bars +
      '<line x1="2" y1="46" x2="290" y2="46" stroke="currentColor" stroke-width="1" opacity=".25"/>' +
      ticks +
      '<text x="290" y="57" text-anchor="end" font-family="JetBrains Mono, monospace" font-size="8" fill="#6E6A61">23</text>' +
      "</svg>";
    var leg =
      '<p class="legend">' + total + " items today. The orange column is the hour you are in.</p>";
      ...
      Object.keys(GRADE)
        .map(function (k) {
          return (
            ...
            esc(GRADE[k].word) +
            "</b>" +
            (compact ? "" : '<span class="def">' + esc(GRADE[k].def) + "</span>") +
            "</div>"
          );
        })
        .join("") +
  ...
  function rail(n) {
    return '<div class="rail"><b>' + (n < 10 ? "0" + n : n) + "</b></div>";
  }
  ...
  function card(st, n, opts) {
    ...
    var also = st.also.length
      ? "<span>\u00b7</span><span>+" + st.also.length + " others</span>"
      : "";
    var body =
      '<div class="card-top"><span class="type">' +
      esc(cap(st.type)) +
      "</span>" +
      verChip(st.verification) +
      "</div>" +
      '<h3 class="headline"><button class="open" data-story="' +
      st.id +
      '">' +
      esc(st.headline) +
      "</button></h3>" +
  ...
  function screenToday() {
    ...
    list.forEach(function (st, i) {
      html += card(st, i + 1, {
        lead: i === 0,
        why: i < (state.mode === 5 ? 1 : 3),
        spark: i === 0,
      });
      if (i === 1)
        html +=
          '<div class="flag">\u25b6 You stopped here \u00b7 ' + (list.length - 2) + " left</div>";
    });
    return html;
  }
  ...
  function screenRadar() {
    ...
    var list = S.filter(function (s) {
      return state.filter === "all" || s.type === state.filter;
    })
      .slice()
      .sort(function (a, b) {
    ...
    var html =
      '<div class="head"><div class="day">Everything arriving</div><h1>Live Radar</h1>' +
      '<p class="sub">' +
      totalArrivals() +
      " items today \u00b7 last checked 3 minutes ago</p></div>" +
      arrivals() +
    ...
    if (!list.length) {
      html +=
        '<div class="empty">Nothing of this kind has arrived today. The worker checks ' +
        "every 30 minutes; the last sweep found nothing matching.</div>";
    }
    list.forEach(function (st) {
      html +=
        '<button class="tick" data-story="' +
        st.id +
        '"><time>' +
        esc(st.detected) +
        ...
        esc(st.headline) +
        "</span>" +
        '<span class="tmeta"><span class="k">' +
        esc(cap(st.type)) +
        "</span>" +
        '<span class="dot">\u00b7</span>' +
        meter(st.verification) +
        '<span class="g">' +
        esc(GRADE[st.verification].word) +
        "</span>" +
        '<span class="dot">\u00b7</span><span class="o">' +
        esc(st.source.name) +
  ...
  function screenStory() {
    ...
    var tl = [{ t: st.detected, s: st.source.name + " publishes first." }].concat(
      st.also.map(function (a) {
        return { t: a.at, s: a.name + " follows" + (a.note ? ", and " + a.note : "") + "." };
      }),
    );
    var bars = st.ranking
      .map(function (r) {
      ...
      .join("");
    return (
      '<div class="railstrip"></div><button class="back" data-back="1">\u2190 Today\u2019s brief</button>' +
      '<div class="sheet"><div class="card-top"><span class="type">' +
      esc(cap(st.type)) +
      "</span>" +
      verChip(st.verification) +
      "</div><h1>" +
      ...
      esc(st.summary) +
      "</p></div>" +
      '<p class="seclab">Why it matters</p><p class="prose" style="color:var(--ink)">' +
      esc(st.why) +
      "</p>" +
      ...
      st.keyPoints
        ...
        .join("") +
      "</ul>" +
      '<p class="seclab">Pickup over the day</p>' +
      spark(st.pickup, true) +
      '<p class="seclab">How this was graded</p><p class="prose">' +
      esc(st.grading) +
      "</p>" +
      '<p class="seclab">Timeline</p><ul class="tl">' +
      tl
        .map(function (e) {
          return "<li><time>" + esc(e.t) + "</time><span>" + esc(e.s) + "</span></li>";
        })
        .join("") +
      "</ul>" +
      ...
      st.also
        ...
        .join("") +
      "</div>" +
      '<p class="seclab">Why this ranked ' +
      (st.rank === 1 ? "first" : "number " + st.rank) +
      '</p><dl class="bars">' +
      bars +
      "</dl></div>"
    );
  ...
  function screenSaved() {
    if (!state.saved.length) {
      return (
        '<div class="head"><div class="day">The bin</div><h1>Saved</h1>' +
    ...
    var html =
      '<div class="railstrip"></div><div class="head"><div class="day">The bin</div><h1>Saved</h1>' +
      '<p class="sub">' +
      state.saved.length +
      " on the pins \u00b7 " +
      state.saved.reduce(function (p, id) {
        return p + byId(id).minutes;
      }, 0) +
      " minutes</p></div>";
    groups.forEach(function (g) {
      ...
      g.ids.forEach(function (id) {
        var st = byId(id);
        html +=
          '<button class="pin" data-story="' +
          st.id +
          '"><span class="hook"></span><span>' +
          "<h3>" +
          esc(st.headline) +
          "</h3>" +
          '<span class="meta"><b>' +
  ...
  function screenSettings() {
    var html =
      '<div class="railstrip"></div><div class="head"><div class="day">AI Radar</div><h1>Settings</h1>' +
      '<p class="sub">Self-hosted \u00b7 ingests every 30 minutes</p></div>';

    html +=
      '<div class="group"><h2>What you care about</h2>' +
    ...
    html +=
      '<div class="group"><h2>Your brief</h2>' +
    ...
    html +=
      '<div class="group"><h2>How you hear about it</h2><div class="radios">' +
      [
        [
          "brief",
          "One push when the brief is ready",
          "A single notification at your chosen time. Nothing else all day.",
        ],
        [
          "urgent",
          "That, plus anything major",
          "Adds a push for a primary-source release from a lab you follow.",
        ],
        [
          "none",
          "Nothing, I will open it myself",
          "No notifications. The brief is waiting when you arrive.",
        ],
      ]
        .map(function (r) {
          return (
            '<label><input type="radio" name="notify" value="' +
            r[0] +
            '"' +
            (state.notify === r[0] ? " checked" : "") +
            "><span>" +
            esc(r[1]) +
            "<small>" +
            esc(r[2]) +
    ...
    html +=
      ...
      SRC.map(function (s, i) {
        return (
          ...
          esc(s.kind) +
          "</span>" +
          '<button class="sw" data-src="' +
          i +
          '" aria-pressed="' +
          !!state.sources[i] +
          '" aria-label="' +
          (state.sources[i] ? "Turn off " : "Turn on ") +
          esc(s.name) +
          '"></button></div>'
        );
      }).join("") +
    ...
    html +=
      '<div class="group"><h2>What the grades mean</h2>' +
    ...
    html +=
      '<div class="group"><h2>First run</h2>' +
  ...
  function screenOnboarding() {
    var steps = [
      ...
      },
      {
        ...
        body:
          '<div class="field"><label for="obt">Ready by</label><input id="obt" type="time" value="07:00"></div>' +
        ...
        body:
          ...
          [
            [
              "brief",
              "One push when the brief is ready",
              "A single notification. Nothing else all day.",
            ],
            [
              "urgent",
              "That, plus anything major",
            ...
            .map(function (r) {
              return (
                '<label><input type="radio" name="obnotify" value="' +
                r[0] +
                '"' +
                (state.notify === r[0] ? " checked" : "") +
                "><span>" +
    ...
    return (
      '<div class="ob"><div class="step">Step ' +
      state.obStep +
      " of 3</div>" +
      "<h1>" +
      esc(s.t) +
      ...
      (state.obStep === 3 ? "Build my first brief" : "Continue") +
      "</button>" +
      '<span class="dots">' +
      [1, 2, 3]
        .map(function (i) {
          return '<i class="' + (i === state.obStep ? "on" : "") + '"></i>';
        })
        .join("") +
      "</span></div></div>"
    );
  ...
  function sidebar() {
    return (
      '<aside class="side"><div class="brand"><span class="mark"></span><b>AI Radar</b></div>' +
      "<nav>" +
      NAV.map(function (n) {
        var count =
          n[0] === "today"
        ...
        return (
          ...
          n[0] +
          '"' +
          (state.screen === n[0] || (n[0] === "today" && state.screen === "story")
            ? ' aria-current="page"'
            : "") +
          ">" +
          n[1] +
      ...
      "</nav>" +
      '<div class="keybox"><p class="cap">How sure are we</p>' +
      gradeKey(true) +
      "</div></aside>"
    );
  }
  ...
  function aside() {
    if (state.screen === "today") {
      ...
      var rows = Object.keys(tally)
        .map(function (k) {
          return (
            ...
            Math.round((tally[k] / shown.length) * 100) +
            '%"></i></div><dd>' +
            tally[k] +
            "</dd></div>"
          );
      ...
      return (
        '<div class="aside"><div class="group"><h2>How today grades out</h2>' +
        '<p class="hint">' +
        confident +
        " of " +
        shown.length +
    ...
    if (state.screen === "radar") {
      return (
        '<div class="aside"><div class="group"><h2>Your topics</h2>' +
        '<p class="hint">Radar shows everything. These only tilt the brief.</p><div class="chips">' +
        state.interests
          .map(function (t) {
            return '<button aria-pressed="true">' + esc(t) + "</button>";
          })
          .join("") +
        "</div></div></div>"
      );
  ...
  function render() {
    ...
    var inner = desktop
      ? sidebar() +
        '<div class="main"><div class="screen"><div class="colwrap">' +
        '<div class="col">' +
        body +
        "</div>" +
        aside() +
  ...
  document.addEventListener("click", function (e) {
    ...
    var t = e.target.closest(
      "[data-story],[data-goto],[data-mode],[data-filter],[data-back],[data-topic],[data-src],[data-ob]",
    );
    if (!t || !document.getElementById("app").contains(t)) return;
    if (t.hasAttribute("data-story")) {
      state.story = t.getAttribute("data-story");
      state.screen = "story";
    } else if (t.hasAttribute("data-back")) {
      state.screen = "today";
    } else if (t.hasAttribute("data-goto")) {
      state.screen = t.getAttribute("data-goto");
    } else if (t.hasAttribute("data-mode")) {
      var m = t.getAttribute("data-mode");
      state.mode = m === "all" ? "all" : +m;
    } else if (t.hasAttribute("data-filter")) {
      state.filter = t.getAttribute("data-filter");
    } else if (t.hasAttribute("data-topic")) {
      var tp = t.getAttribute("data-topic"),
        i = state.interests.indexOf(tp);
      if (i > -1) state.interests.splice(i, 1);
      else state.interests.push(tp);
